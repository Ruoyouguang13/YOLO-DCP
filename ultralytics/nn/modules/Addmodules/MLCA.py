...
class MLCA(nn.Module):
    def __init__(self, in_size, local_size=5, gamma=2, b=1, local_weight=0.5):
        ...
        t = int(abs(math.log(in_size, 2) + self.b) / self.gamma)  # eca  gamma=2 根据输入通道数动态计算卷积核大小 k
        k = t if t % 2 else t + 1   #确保 k 是奇数（便于对称 padding）
        ...
        self.local_arv_pool = nn.AdaptiveAvgPool2d(local_size)    #计算局部区域的均值
        self.global_arv_pool = nn.AdaptiveAvgPool2d(1)          #计算全局均值
