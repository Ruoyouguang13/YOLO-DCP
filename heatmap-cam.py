...
class ActivationsAndGradients:
    ...
    def __call__(self, x):
        ...
        if self.model.task == 'detect':
            post_result, pre_post_boxes = self.post_process(model_output['one2many'][0])
...
class yolo_heatmap:
    ...
    def process(self, img_path, save_path):
        ...
        tensor = tensor.to('cpu')  # ⚠️ 不推荐：会影响性能
        ...
        # 手动 forward 推理（不再调用 predict）
        with torch.no_grad():
            model_output = self.model_yolo.model(tensor)

        # 获取预测框
        pred_tensor = model_output['one2one'][0]  # (num_boxes, 6) 或 (num_boxes, 85)['one2many'][0]
        pred = self.post_process(pred_tensor)  # 非最大抑制后的预测框

        # 可选：只在目标框内归一化热力图
        if self.renormalize and self.task in ['detect', 'segment', 'pose']:
            boxes = pred[:, :4].cpu().numpy().astype(np.int32)  # xyxy 坐标
            cam_image = self.renormalize_cam_in_bounding_boxes(boxes, img, grayscale_cam)

        # 可选：可视化目标框
        if self.show_result:
            # for i, box in enumerate(pred[:, :4]):
            #     cam_image = self.draw_detections(box, self.colors[0], 'film', cam_image)
            for i in range(len(pred)):
                box = pred[i, :4].cpu().numpy().astype(np.int32)
                cls_id = int(pred[i, 5])
                cls_name = self.model_names[cls_id]
                color = self.colors[cls_id]
                cam_image = self.draw_detections(box, color, cls_name, cam_image)  #coco代码
...
def get_params():
    params = {
        'weight': r"C:\Users\zgq\Desktop\model\best.pt",
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    model(r"C:\Users\zgq\Desktop\0000007_04999_d_0000036.jpg", 'result')
